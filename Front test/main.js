    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.token) {
                ...
                // Déclare userInfo avec les informations de l'utilisateur
                userInfo = data.user;
...
function activateHomeTab(userInfo, isLoggedIn) {
    ...
    // Affiche le contenu de l'onglet "Accueil"
...
async function fetchData() {
    try {
        ...
        // Retourne les données des posts plutôt que de stocker dans une variable globale
        return data;
    } catch (error) {
...
async function addComment(event, postId) {
    ...
    try {
        ...
        // Utilise la fonction fetchAndDisplayComments pour obtenir les données mises à jour des commentaires
        await fetchAndDisplayComments(postId);
    
        // Efface le champ du formulaire
...
async function addPost(event) {
    ...
    try {
        ...
        // Utilise la fonction fetchData pour obtenir les données mises à jour des posts
        fetchData().then((data) => {
            displayPosts(data);
        });
...
async function likePost(postId) {
    ...
    try {
        ...
        // Récupére les données mises à jour des posts après le like
        const updatedPosts = await fetchData();

        // Mets à jour votre interface utilisateur en conséquence avec les nouveaux posts
...
async function dislikePost(postId) {
    ...
    try {
        ...
        // Mets à jour interface utilisateur en conséquence
        displayPosts(updatedPosts); // Mise à jour des post existant
    } catch (error) {
...
async function editPost(postId) {
    ...
    try {
        // Récupére le contenu actuel du post
        ...
        // Ferme la fenêtre modale après l'édition
        $('#editPostModal').modal('hide');

        // Récupére les données mises à jour des posts après l'édition
        const updatedPosts = await fetchData();

        // Mise à jour de l'interface utilisateur en conséquence avec les nouveaux posts
        displayPosts(updatedPosts);
    } catch (error) {
...
async function deletePost(postId) {
    try {
      ...
      if (response.ok) {
        ...
        // Mise à jour de interface utilisateur en conséquence
...
// Fonction pour ajouter un écouteur d'événements 
    ...
    if (posts && Array.isArray(posts)) {
        posts.forEach((post) => {
            const postElement = createElement("div", { class: "col-md-8 offset-md-2 mb-3" }, `
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="card-text">
                            <small class="text-muted">${post.author && post.author.username ? post.author.username : 'Unknown Author'}</small></br>
                            <small class="text-muted">${post.createdAt}</small></br>
                            <small class="text-primary likers">Liked by: ${post.likers ? post.likers.map(liker => liker.userId.username).join(', ') : 'None'}</small></br>
                            <small class="text-danger dislikers">Disliked by: ${post.dislikers ? post.dislikers.map(disliker => disliker.userId.username).join(', ') : 'None'}</small>
                        </p>
                    </div>
                    ...
                    <div class="comments-section">
                        <h5>Commentaires</h5>
                        <div id="commentContainer-${post._id}"></div> 
            ...
            if (post.author._id === userInfo._id) {
                ...
                // Ajoute l'écouteur d'événements pour le bouton "Éditer"
                editButton.addEventListener("click", () => {
                    ...
                    // Remplis le champ de saisie du modal avec le contenu actuel
                    const editedPostContent = document.getElementById('editedPostContent');
                    editedPostContent.value = currentContent;
                
                    // Stocke l'ID du post en cours d'édition dans un attribut du bouton "Enregistrer les modifications"
            ...
            if (post.author._id === userInfo._id) {
                ...
                // Ajoute l'écouteur d'événements pour le bouton de suppression
                deleteButton.addEventListener("click", async () => {
                  try {
                    const postId = deleteButton.getAttribute("data-post-id");
                    console.log("Supprimer le bouton cliqué pour le post ID:", postId);
              
                    // Appel la fonction de suppression côté client
                    await deletePost(postId);
              
                    
              
                    // supprime l'élément du post du DOM si nécessaire
                    postElement.remove();
                  } catch (error) {
                ...
                // Ajoute le bouton de suppression à l'élément du post
